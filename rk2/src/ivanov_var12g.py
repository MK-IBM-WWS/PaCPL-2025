...
class ProgrammingSystem:
    ...
    def get_one_to_many(self):
        return [(tool.name, tool.license_cost, lang.name)
    ...
    def get_many_to_many(self):
        many_to_many_temp = [(lang.name, lt.lang_id, lt.tool_id)
                             for lang in self.languages
                             for lt in self.langs_tools
                             if lang.id == lt.lang_id]

        return [(tool.name, tool.license_cost, lang_name)
                for lang_name, lang_id, tool_id in many_to_many_temp
                for tool in self.tools if tool.id == tool_id]

    def get_tools_starting_with_v(self):
        one_to_many = self.get_one_to_many()
        return [(tool_name, lang_name)
                for tool_name, _, lang_name in one_to_many
                if tool_name.startswith('V')]

    def get_languages_with_max_license_cost(self):
        one_to_many = self.get_one_to_many()
        ...
        return sorted(res_g2_unsorted, key=lambda x: x[2], reverse=True)

    def get_all_linked_tools_sorted(self):
